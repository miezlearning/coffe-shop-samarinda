    ...
    >
      ...
      <div className="relative h-56 overflow-hidden bg-zinc-800">
        ...
        <div className="absolute top-4 left-4">
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${style.bg} ${style.text} text-xs font-bold shadow-lg`}>
            <LabelIcon size={12} />
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <h3 className="text-white font-extrabold text-xl leading-tight">{shop.name}</h3>
          <p className="text-zinc-400 text-sm mt-1 line-clamp-1 italic">{shop.tagline}</p>
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col gap-3">
            ...
            <div className="flex">
              {[1,2,3,4,5].map((s) => (
                <Star
                  key={s}
                  size={13}
                  className={s <= Math.round(shop.rating) ? 'fill-cream-400 text-cream-400' : 'text-zinc-700'}
                />
              ))}
            </div>
            <span className="text-zinc-400 text-xs">({shop.reviewCount.toLocaleString('id-ID')} ulasan)</span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2 p-2.5 rounded-xl bg-white/4 border border-white/8">
          ...
          <div>
            <div className="text-xs text-zinc-600 font-medium uppercase tracking-wide">Wajib Coba</div>
            <div className="text-sm text-zinc-300 font-medium mt-0.5">{shop.mustTry}</div>
          </div>
        </div>
        ...
        >
          ...
          <ExternalLink size={13} className="opacity-70 group-hover/btn:opacity-100" />
        </a>
      </div>
    </motion.div>
  )
        ...
        >
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-2 max-w-lg">
              Kafe-kafe yang paling direkomendasikan, dikurasi khusus berdasarkan rating dan popularitas.
            </p>
          </div>

          <div className="hidden sm:flex items-center gap-2 shrink-0">
            <div className="w-2 h-2 rounded-full bg-coffee-400 animate-pulse" />
            <span className="text-xs text-zinc-600">{featured.length} kafe pilihan</span>
          </div>
        </motion.div>
